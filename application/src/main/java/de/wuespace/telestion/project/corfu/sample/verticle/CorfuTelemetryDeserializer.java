...
import de.wuespace.telestion.project.corfu.sample.verticle.message.DeserializerCommand;
import de.wuespace.telestion.project.corfu.sample.verticle.message.DeserializerResetCommand;
...
public class CorfuTelemetryDeserializer extends TelestionVerticle<CorfuTelemetryDeserializer.Configuration>
	...
	public record Configuration(
		...
		public Configuration() {
			this(null, null, "corfu/deserializer/status", "corfu/deserializer/command");
		}
	}
	...
	public void handle(RawMessage message) {
		logger.debug("Deserialize new telemetry");

		try {
			var telemetry = GlobalSerializer.serializer().deserialize(message.data());
			publish(getConfig().outAddress, telemetry);
			incrementCounter(Counter.GOOD);
		} catch (PacketToSmallException e) {
			incrementCounter(Counter.BAD_PACKET_TO_SMALL);
		} catch (PacketToLargeException e) {
